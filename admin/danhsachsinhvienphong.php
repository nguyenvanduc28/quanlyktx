...
if ($_SERVER['REQUEST_METHOD'] == 'GET' and !empty($_GET['id'])) {
    $idPhong = !empty($_GET['id']) ? $_GET['id'] : null;
    if (!empty($idPhong)) {
        $danhsachcungphong = $dangkyphong->getDanhsachsinhvienphong($idPhong);
    }
...
<head>
    ...
    <link rel="stylesheet" href="./css/danhsachsinhvienphong.css">
...
<body>
    ...
    <div class="danhsachsinhvienphong-root">
        <h1>Danh sách sinh viên phòng</h1>
        <table class="table">
            <thead>
                <tr>
                    <th scope="col">STT</th>
            ...
            <tbody>
                <?php if(is_array($danhsachcungphong)) {foreach ($danhsachcungphong as $key => $value) { ?>
                    <tr>
                        ...
                        <td><?= $value['lop'] ?></td>
                        <td><?= $value['xacnhan'] ? 'Đã xác nhận' : 'Chưa xác nhận' ?></td>
                    </tr>
                <?php }} ?>
            </tbody>
        </table>
